...
export const useAnalyticsData = ({
  ...
  const [analyticsData, setAnalyticsData] = useState<SystemAnalytics | null>(
    null,
  );
  ...
  const filteredHistory = useMemo(
    () => history.filter((record) => record.systemId === selectedSystemId),
    [history, selectedSystemId],
  );
  ...
  const visibleAlerts = useMemo(() => {
    ...
    filteredHistory.forEach((record: AnalysisRecord) => {
      const recordTime = new Date(record.timestamp).getTime();

      // Filter by visible time range if provided
      if (visibleTimeRange) {
        if (
          recordTime < visibleTimeRange.start ||
      ...
      if (record.analysis?.alerts && Array.isArray(record.analysis.alerts)) {
        record.analysis.alerts.forEach((alert: string) => {
  ...
  }, [analyticsData, filteredHistory, visibleTimeRange]);
