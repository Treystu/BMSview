...
async function buildGuruPrompt({ analysisData, systemId, customPrompt, log, context, mode = "sync", insightMode = "with_tools" }) {
    ...
    if (customPrompt) {
        ...
        prompt += "• Keep text SHORT and punchy - bullet points over paragraphs\n";
        prompt += "• Include short affirmative phrases about visual clarity, e.g.:\n";
        prompt += "  - 'This chart makes the charge pattern easy to see'\n";
        prompt += "  - 'These visuals highlight the most important trends at a glance'\n";
        prompt += "  - 'The infographic above summarizes your battery health'\n\n";
        prompt += "**📈 CHART REQUIREMENTS (MANDATORY for time-based data):**\n";
        prompt += "For ANY time-series insight (SOC over time, temperature trends, charge/discharge cycles), you MUST:\n";
        prompt += "1. CALL request_bms_data to fetch the time-series data\n";
        prompt += "2. Include a CHART CONFIGURATION block in your response\n\n";
        prompt += "**CHART CONFIGURATION FORMAT:**\n";
        prompt += "Use this exact JSON structure inside a ```chart code block:\n";
        prompt += "```chart\n";
        prompt += "{\n";
        ...
        prompt += "  \"series\": [\n";
        prompt += "    {\n";
        prompt += "      \"name\": \"State of Charge\",\n";
        prompt += "      \"data\": [[\"2025-11-29T00:00:00Z\", 85], [\"2025-11-29T06:00:00Z\", 72], ...]\n";
        prompt += "    }\n";
        prompt += "  ],\n";
        ...
        prompt += "• `stacked_bar` - Daily charge vs discharge breakdown\n";
        prompt += "• `area` - Cumulative energy over time\n";
        prompt += "• `gauge` - Current SOC, health score, efficiency rating\n\n";
        prompt += "**GAUGE CONFIGURATION (for single-value metrics):**\n";
        prompt += "```chart\n";
        prompt += "{\n";
        ...
        prompt += "  \"max\": 100,\n";
        prompt += "  \"thresholds\": [\n";
        prompt += "    { \"value\": 50, \"color\": \"red\", \"label\": \"Poor\" },\n";
        prompt += "    { \"value\": 75, \"color\": \"yellow\", \"label\": \"Fair\" },\n";
        prompt += "    { \"value\": 90, \"color\": \"green\", \"label\": \"Good\" }\n";
        prompt += "  ],\n";
        prompt += "  \"insights\": \"Your battery health is above average!\"\n";
        prompt += "}\n";
        prompt += "```\n\n";
        prompt += "**INFOGRAPHIC STRUCTURE (use this layout):**\n";
        prompt += "## 📊 VISUAL SUMMARY\n";
        prompt += "[Gauge or status card for current system state]\n\n";
        prompt += "## 📈 KEY TRENDS\n";
        prompt += "[Line/area chart for primary time-series metric]\n";
        prompt += "[Short explanation with visual reference]\n\n";
        prompt += "## ⚡ ENERGY FLOW\n";
        prompt += "[Stacked bar or bar chart for charge/discharge]\n";
        prompt += "[Callout boxes for key numbers]\n\n";
        prompt += "## 🔔 ALERTS & STATUS\n";
        prompt += "[Status badges: 🟢 OK | 🟡 Warning | 🔴 Critical]\n";
        prompt += "[Short bullet list of current status items]\n\n";
        prompt += "## 📋 RECOMMENDATIONS\n";
        prompt += "[Numbered list with severity icons]\n\n";
        prompt += "**IF NO TIME-SERIES DATA AVAILABLE:**\n";
        prompt += "• State clearly: 'No time-series data available for chart generation'\n";
        prompt += "• Focus on other visual elements: status badges, comparison blocks, gauge charts for current values\n";
        prompt += "• Still use the infographic structure above\n\n";
        prompt += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n";
    } else {
