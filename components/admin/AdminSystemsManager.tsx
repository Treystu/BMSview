...
const AdminSystemsManager: React.FC<AdminSystemsManagerProps> = ({ 
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const systemToSave: BmsSystem = {
                ...
                associatedDLs: formData.associatedDLs || [],
            };
    ...
    const handleAddDL = () => {
        if (dlInput.trim()) {
            const updatedDLs = [...(formData.associatedDLs || []), dlInput.trim()];
            setFormData({ ...formData, associatedDLs: updatedDLs });
            setDlInput('');
        }
    };

    const handleRemoveDL = (dlToRemove: string) => {
        const updatedDLs = (formData.associatedDLs || []).filter(dl => dl !== dlToRemove);
        setFormData({ ...formData, associatedDLs: updatedDLs });
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-neutral-dark rounded-lg shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <section>
                        <h3 className="text-lg font-semibold text-secondary mb-3">Associated DL Numbers</h3>
                        <div className="space-y-3">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddDL())}
                            ...
                            {formData.associatedDLs && formData.associatedDLs.length > 0 && (
                                <div className="flex flex-wrap gap-2">
                                    {formData.associatedDLs.map((dl, index) => (
                                        <div key={index} className="bg-gray-800 px-3 py-1 rounded flex items-center gap-2">
                                            <span className="font-mono text-sm">{dl}</span>
                                            <button
                                                type="button"
