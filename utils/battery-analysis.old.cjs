 ...
 * @property {string} [systemId] - Battery system identifier
 */
...
/**
 * @typedef {Object} RuntimeEstimate
 * @property {number|null} runtimeHours - Estimated runtime in hours
 * @property {string} explanation - Explanation of the estimate
 * @property {string} confidence - Confidence level of the estimate (low, medium, high)
 */
...
const calculateCapacityRetention = (measurements) => {
  if (!measurements || measurements.length < 2) return 100;
  ...
  return (latest.capacity / first.capacity) * 100;
};
...
/**
 * Calculate estimated runtime based on battery measurements
 * @param {Measurement[]} measurements - Battery measurements
 * @param {Object} lastKnown - Last known battery state
 * @param {number} [lastKnown.capacity] - Capacity in Ah
 * @param {number} [lastKnown.capacityAh] - Capacity in Ah
 * @param {number} [lastKnown.stateOfCharge] - State of charge percentage
 * @param {number} [lastKnown.voltage] - Voltage
 * @returns {RuntimeResult} Runtime estimate
 */
function calculateRuntimeEstimate(measurements, lastKnown) {
  const result = {
    runtimeHours: 12, // Default to conservative 12 hours
  ...
  // Calculate capacity and voltage from measurements if not provided
  const capacityAh = lastKnown?.capacityAh || lastKnown?.capacity ||
    measurements[measurements.length - 1]?.capacity || 100;
  const soc = lastKnown?.stateOfCharge ?? lastKnown?.soc ??
    measurements[measurements.length - 1]?.stateOfCharge ?? 100;
  const voltage = lastKnown?.voltage ||
    measurements[measurements.length - 1]?.voltage || 48;

  // Get discharge measurements
  const discharge = measurements.filter(m => (m.current || 0) < 0 && (m.voltage || 0) > 0);
  let avgPowerW = null;

  if (discharge.length) {
    // Calculate average discharge power
    const powers = discharge.map(d => Math.abs((d.current || 0) * (d.voltage || 0)));
  ...
  function determineHealthStatus(measurements, maxTemp) {
    ...
    if (maxTemp > 45 || capacityRetention < 60) return 'Critical';

    // Then check combinations of retention and SoC
    if (capacityRetention >= 90) return avgSoC >= 80 ? 'Excellent' : 'Good';
    if (capacityRetention >= 80) return avgSoC >= 70 ? 'Good' : 'Fair';
    if (capacityRetention >= 70) return avgSoC >= 60 ? 'Fair' : 'Poor';

    // Default to poor if none of the above conditions are met
    return 'Poor';
  }
  ...
  function calculatePerformance(measurements, timeSpan) {
    ...
    const degradationPerDay = timeSpan ? ((100 - capacityRetention) / timeSpan) : 0;
    const degradationRate = Math.max(0, Math.min(10, degradationPerDay)); // Cap at 10% per day
  ...
  /**
   * Calculate charging efficiency based on measurements
   * @param {Measurement[]} measurements Battery measurements
   * @returns {number} Efficiency as percentage (0-100)
   */
  function calculateChargeEfficiency(measurements) {
    if (!measurements?.length) return 0;

    const chargeCycles = measurements.filter(m => m?.current > 0 && m?.voltage > 0);
    if (!chargeCycles.length) return 95; // Default to 95% if no data

    // Calculate energy input during charging
    const energyIn = chargeCycles.reduce((sum, m) => sum + (m.current * m.voltage), 0);

    // Get discharge cycles
    const dischargeCycles = measurements.filter(m => m?.current < 0 && m?.voltage > 0);
    if (!dischargeCycles.length) return 95;

    // Calculate energy output during discharge
    const energyOut = Math.abs(dischargeCycles.reduce((sum, m) => sum + (m.current * m.voltage), 0));

    if (energyIn === 0) return 95;

    // Calculate efficiency as energy out / energy in
    let efficiency = (energyOut / energyIn) * 100;

    // Bound efficiency between 70% and 99%
    efficiency = Math.max(70, Math.min(99, efficiency));

    return Math.round(efficiency);
  }

  /**
   * Calculate discharging efficiency based on measurements
   * @param {Measurement[]} measurements Battery measurements
   * @returns {number} Efficiency as percentage (0-100)
   */
  function calculateDischargeEfficiency(measurements) {
    ...
    const dischargeCycles = measurements.filter(m => m?.current < 0 && m?.voltage > 0);
    if (!dischargeCycles.length) return 95; // Default to 95% if no data

    // Calculate actual vs theoretical energy during discharge
    const actualEnergy = dischargeCycles.reduce((sum, m) => sum + Math.abs(m.current * m.voltage), 0);
    const theoreticalEnergy = dischargeCycles.reduce((sum, m) => sum + Math.abs(m.current * m.voltage * 1.05), 0);

    if (theoreticalEnergy === 0) return 95;

    // Calculate efficiency
    let efficiency = (actualEnergy / theoreticalEnergy) * 100;
