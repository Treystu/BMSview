...
export const __internals = {
  ...
  setApiFetch: (fn: typeof _apiFetchImpl) => {
    _apiFetchImpl = fn;
  },
  resetApiFetch: () => {
    _apiFetchImpl = defaultApiFetch;
  },
...
async function getCachedSystemsPage(
  page: number,
  limit: number | "all",
): Promise<PaginatedResponse<BmsSystem> | null> {
...
async function getCachedHistoryPage(
  page: number,
  limit: number | "all",
): Promise<PaginatedResponse<AnalysisRecord> | null> {
  if (!isLocalCacheEnabled()) {
    recordCacheDisabledSkip();
    return null;
  }
...
function setCachedSystemsPage(
  page: number,
  limit: number | "all",
  data: PaginatedResponse<BmsSystem>,
): void {
...
function setCachedHistoryPage(
  page: number,
  limit: number | "all",
  data: PaginatedResponse<AnalysisRecord>,
): void {
  if (!isLocalCacheEnabled()) return;
  const key = `history:${page}:${limit}`;
...
export const setCacheDisabled = (disabled: boolean) => {
  if (typeof window !== "undefined") {
    ...
    if (typeof globalThis !== "undefined") {
      (
        globalThis as typeof globalThis & { __BMSVIEW_DISABLE_CACHE?: boolean }
      ).__BMSVIEW_DISABLE_CACHE = disabled;
    }
...
async function defaultApiFetch<T>(
  endpoint: string,
  options: RequestInit = {},
): Promise<T> {
  ...
  try {
    ...
    if (response.status === 204 || contentLength === "0") {
      log("info", "API fetch successful with no content.", {
        ...logContext,
        status: response.status,
      });
      return null as T;
...
let _apiFetchImpl: <T>(endpoint: string, options?: RequestInit) => Promise<T> =
  defaultApiFetch;
...
export const getRegisteredSystems = async (
  page = 1,
  limit: number | "all" = 100,
  options: FetchListOptions = {},
): Promise<PaginatedResponse<BmsSystem>> => {
  const { forceRefresh = false, strategy = FetchStrategy.CACHE_FIRST } =
    options;
  const isForceRefresh = forceRefresh || strategy === FetchStrategy.FORCE_FRESH;
  log("info", "Fetching paginated registered BMS systems.", {
    page,
    limit,
    strategy,
  });
  ...
  const response = isForceRefresh
    ? await apiFetch<PaginatedResponse<BmsSystem>>(
        `systems?page=${page}&limit=${limit}`,
      )
    : await fetchWithCache<PaginatedResponse<BmsSystem>>(
        `systems?page=${page}&limit=${limit}`,
        10_000,
      );
  ...
  if (Array.isArray(response)) {
    ...
    const totalItems = totalCandidates.find(
...
export const getAnalysisHistory = async (
  ...
  const isForceRefresh = forceRefresh || strategy === FetchStrategy.FORCE_FRESH;
  log("info", "Fetching paginated analysis history.", {
    page,
    limit,
    strategy,
    sortBy,
  ...
  if (!isForceRefresh) {
    ...
    if (cached) {
      log("info", "Serving analysis history from cache.", {
  ...
  try {
    ...
    log("info", "Normalized analysis history response.", {
 ...
 */
export const generateInsightsBackground = async (payload: {
  ...
  try {
    ...
    log("error", "Failed to start background insights", {
      error: error.message,
    });
    throw error;
  }
...
export const getInsightsJobStatus = async (jobId: string): Promise<unknown> => {
  ...
  try {
    const response = await fetch(
      "/.netlify/functions/generate-insights-status",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const error = err instanceof Error ? err : new Error(String(err));
    log("error", "Failed to get insights job status", {
      jobId,
      error: error.message,
    });
    throw error;
...
export const streamInsights = async (
  ...
  const hasRecentHistory =
    Array.isArray(payload.recentHistory) && payload.recentHistory.length > 0;
  ...
  log("info", "Streaming insights from server.", {
    ...
    insightMode: mode,
    dataStructure: payload.analysisData
      ? Object.keys(payload.analysisData)
      : "none",
    hasRecentHistory,
  ...
  try {
    while (attemptCount < MAX_RESUME_ATTEMPTS) {
      ...
      const timeoutId = setTimeout(() => {
        ...
        log(
          "warn",
          `Insights request timed out after ${REQUEST_TIMEOUT_MS}ms.`,
        );
      }, REQUEST_TIMEOUT_MS);
    ...
    throw new Error(
      `Insights generation failed after ${MAX_RESUME_ATTEMPTS} attempts`,
    );
  } catch (error) {
    const err = error instanceof Error ? error : new Error(String(error));
...
export const getRecentHistory = async (
  ...
  try {
    ...
    log("warn", "Failed to retrieve recent history from local cache.", {
      error: String(error),
    });
    return [];
  }
...
export const getRecentHistoryForSystem = async (
  systemId: string,
  limit = 10,
): Promise<AnalysisRecord[]> => {
...
export const getSystemAnalytics = async (
  ...
  const history = await getAnalysisHistory(1, "all", {
    strategy: FetchStrategy.FORCE_FRESH,
  });
...
export const addVerifiedRange = async (
  range: string,
): Promise<{ verifiedRanges: string[] }> => {
...
export const removeVerifiedRange = async (
  range: string,
): Promise<{ verifiedRanges: string[] }> => {
...
export const addBlockedRange = async (
  range: string,
): Promise<{ blockedRanges: string[] }> => {
...
export const removeBlockedRange = async (
  range: string,
): Promise<{ blockedRanges: string[] }> => {
...
export const runDiagnostics = async (
  ...
  try {
    ...
    const summaryObj =
      respObj.summary && typeof respObj.summary === "object"
        ? (respObj.summary as Record<string, unknown>)
        : {};
    const status =
      respObj.status === "success" ||
      respObj.status === "partial" ||
      respObj.status === "warning" ||
      respObj.status === "error"
        ? respObj.status
        : "success";

    const timestamp =
      typeof respObj.timestamp === "string"
        ? respObj.timestamp
        : new Date().toISOString();
    const duration =
      typeof respObj.duration === "number" ? respObj.duration : 0;
    ...
    return {
      ...
      summary: {
        total:
          typeof summaryObj.total === "number"
            ? summaryObj.total
            : normalizedResults.length,
        passed:
          typeof summaryObj.passed === "number"
            ? summaryObj.passed
            : typeof summaryObj.success === "number"
              ? summaryObj.success
              : 0,
        warnings:
          typeof summaryObj.warnings === "number" ? summaryObj.warnings : 0,
        errors: typeof summaryObj.errors === "number" ? summaryObj.errors : 0,
        skipped:
          typeof summaryObj.skipped === "number" ? summaryObj.skipped : 0,
      },
    };
  } catch (error) {
    clearTimeout(timeoutId);
...
export const getDiagnosticProgress = async (
  testId: string,
): Promise<{
...
export const getAdminStories = async (
  ...
  if (options.systemIdentifier)
    params.append("systemIdentifier", options.systemIdentifier);
...
export const deleteAdminStory = async (
  id: string,
): Promise<{ success: boolean; id: string }> => {
...
export const addEventToStory = async (
  storyId: string,
  event: {
    analysisId: string;
    annotation?: string;
...
export const removeEventFromStory = async (
  storyId: string,
  eventIndex: number,
): Promise<{ success: boolean }> => {
  log("info", "Removing event from story.", { storyId, eventIndex });
  return apiFetch(`stories/${storyId}/events/${eventIndex}`, {
    method: "DELETE",
  });
};
...
export const checkHashes = async (
  ...
  duplicates: { hash: string; data: unknown }[];
  upgrades: string[];
}> => {
...
export const runSingleDiagnosticTest = async (
  testScope: string,
...
export const getHourlySocPredictions = async (
  systemId: string,
  hoursBack: number = 72,
): Promise<unknown> => {
  log("info", "Fetching hourly SOC predictions.", { systemId, hoursBack });
  return apiFetch(
    `predictions/hourly-soc?systemId=${systemId}&hoursBack=${hoursBack}`,
  );
};
...
export const getMergedTimelineData = async (
  ...
  if (options.includeWeather !== undefined)
    params.append("includeWeather", String(options.includeWeather));
...
export const syncWeather = async (
  ...
  try {
    ...
    log("warn", "Weather sync failed (endpoint may not exist).", {
      error: String(error),
    });
  }
};
...
export const getUnifiedHistory = async (
  ...
  log("info", "Fetching history records for unified timeline.", {
    systemId,
    isAllData,
  });
  try {
    ...
    if (!isAllData && systemId) {
      ...
      log("info", "System-specific history records fetched.", {
        systemId,
        count: historyRecords.length,
      });
    } else {
      // For 'ALL' case, use the regular endpoint with 'all' limit
      const historyResponse = await getAnalysisHistory(1, "all", {
        strategy: FetchStrategy.FORCE_FRESH,
      });
      historyRecords = historyResponse.items;
      log("info", "All history records fetched.", {
        count: historyRecords.length,
      });
    }
  } catch (error) {
    log("error", "Failed to fetch history for unified timeline.", {
      error: String(error),
    });
  }
  ...
  const unified: UnifiedTimelinePoint[] = [
    ...
    ...weatherRecords.map((w) => ({
      ...
      timestamp:
        (w as { timestamp?: string; time?: string }).timestamp ||
        (w as { timestamp?: string; time?: string }).time ||
        "",
      data: w,
    })),
  ].sort(
...
export const streamAllHistory = async (
  onData: (records: AnalysisRecord[]) => void,
  onComplete: () => void,
): Promise<void> => {
  log("info", "Starting smart history sync.");
  try {
    const response = await apiFetch<
      AnalysisRecord[] | { items: AnalysisRecord[]; totalItems?: number }
    >("history?limit=all");
...
export const mergeBmsSystems = async (
...
export const findDuplicateAnalysisSets = async (): Promise<
  AnalysisRecord[][]
> => {
  const response = await apiFetch<{ duplicateSets?: AnalysisRecord[][] }>(
    "admin-scan-duplicates",
  );
  if (
    response &&
    typeof response === "object" &&
    Array.isArray(response.duplicateSets)
  ) {
...
export const deleteAnalysisRecords = async (
  recordIds: string[],
): Promise<void> => {
...
export const deleteAnalysisRecord = async (recordId: string): Promise<void> => {
  await apiFetch(`history?id=${encodeURIComponent(recordId)}`, {
    method: "DELETE",
  });
};

export const linkAnalysisToSystem = async (
  recordId: string,
  systemId: string,
...
export const countRecordsNeedingWeather = async (): Promise<{
  count: number;
}> => {
...
export const normalizeIds = async (
  ...
  const resp = await apiFetch<{ updatedCount?: number; errors?: string[] }>(
    "history",
    {
      method: "POST",
...
export const fixPowerSigns = async (): Promise<{
  fixed: number;
  errors: string[];
}> => {
  const resp = await apiFetch<{ updatedCount?: number; errors?: string[] }>(
...
export const createAnalysisStory = async (
  storyData: Partial<AnalysisStory>,
): Promise<AnalysisStory> => {
...
export const clearHistoryStore = async (): Promise<{
  message: string;
  details: unknown;
}> => {
...
export const associateHardwareIdToSystem = async (
  hardwareId: string,
  systemId: string,
): Promise<void> => {
...
export const cleanupLinks = async (): Promise<{
  success: boolean;
  updatedCount: number;
}> => {
...
export const autoAssociateRecords = async (
  skip: number = 0,
): Promise<{
  success: boolean;
  processed: number;
...
export const runUnifiedDiagnostics = async (options: {
  testId?: string;
  category?: string;
  action?: "run" | "runAll";
  ...
  return apiFetch("unified-diagnostics", {
    method: "POST",
