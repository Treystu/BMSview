...
const AnalysisHistory: React.FC<AnalysisHistoryProps> = ({
  ...
  const handleLinkRecord = async (
    recordId: string,
    hardwareSystemId: string | null | undefined,
  ) => {
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-neutral mt-2 max-w-2xl mx-auto">
            Review your past analysis results to track your systems&apos; health
            over time.
          </p>
        </div>
                      ...
                      {!record.systemId ? (
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            ...
                            disabled={
                              !linkSelections[record.id] ||
                              linkingStates[record.id]
                            }
                        ...
                        <span className="font-semibold text-neutral-dark">
                          {record.systemName || "Unlinked"}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap font-mono text-gray-600 text-xs">
                      {record.hardwareSystemId || record.dlNumber || "N/A"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-600">
                      V: {record.analysis?.overallVoltage?.toFixed(1) ?? "N/A"}
                      <br />
                      A: {record.analysis?.current?.toFixed(1) ?? "N/A"}
                      <br />
