...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout }) => {
  ...
  const fetchData = useCallback(
    async (
      ...
      try {
        ...
        if (type === "all" || type === "history") {
          ...
          promises.push(
            getAnalysisHistory(page, ITEMS_PER_PAGE, historyOptions),
          );
        }
        ...
        if (type === "all") {
          ...
          payload.systems = responses[0] as {
            items: BmsSystem[];
            total: number;
          };
        } else if (type === "history") {
          payload.history = responses[0] as {
    ...
    [dispatch, state.historySortKey, state.historySortDirection],
  ); // Added sort dependencies
  ...
  const jobPollingConfig: {
    ...
    onComplete: useCallback(
      async (jobId: string, recordId: string) => {
        ...
        try {
          ...
          const completedRecord = historyResponse.items.find(
            (r) => r.id === recordId,
          );
          ...
          log("error", "Failed to fetch completed analysis record", {
            jobId,
            recordId,
            error,
          });
    ...
    onError: useCallback(
      (jobId: string, error: string) => {
        ...
        const failedResult = bulkUploadResults.find(
          (r) =>
            r.data &&
            typeof r.data === "object" &&
            "_recordId" in r.data &&
            r.data._recordId === jobId,
        );
  ...
  useEffect(() => {
    const initialLoad = async () => {
      ...
      try {
        ...
        log("info", "Successfully fetched initial page data.", {
          systemCount: systemsResponse.items.length,
          historyCount: historyResponse.items.length,
        });
        dispatch({
        ...
        setTimeout(() => {
          ...
          streamAllHistory(
            (records) =>
              dispatch({ type: "APPEND_HISTORY_CACHE", payload: records }),
  ...
  const handleBulkAnalyze = async (
    ...
    options: { forceReanalysis?: boolean; useAsync?: boolean } = {},
  ) => {
    ...
    log("info", "Starting bulk analysis.", {
    ...
    if (state.isCacheBuilding) {
      log(
        "warn",
        "Duplicate check may be incomplete - history cache still building.",
        {
    ...
    const initialResults: DisplayableAnalysisResult[] = normalizedFiles.map(
      (f) => ({
        fileName: f.name,
        data: null,
        error: forceReanalysis
          ? "Queued (Force Override)"
    ...
    try {
      ...
      if (forceReanalysis) {
        log(
          "info",
          "Force Re-analysis enabled: Bypassing duplicate checks and treating all files as new.",
        );
        ...
        const cachedUpgradeResults: DuplicateCheckResult[] = cachedUpgrades.map(
          (file) => ({
            file,
            isDuplicate: true,
            needsUpgrade: true,
        ...
        const alreadyCheckedResults: DuplicateCheckResult[] =
          alreadyCheckedNewFiles.map((file) => ({
            file,
            isDuplicate: false,
            needsUpgrade: false,
          }));
      ...
      if (trueDuplicates.length > 0 && !forceReanalysis) {
        ...
        log(
          "info",
          `Skipped ${trueDuplicates.length} duplicates from network check.`,
        );
      }
      ...
      const processFile = async (item: DuplicateCheckResult) => {
        ...
        dispatch({
          type: "UPDATE_BULK_UPLOAD_RESULT",
          payload: { fileName: file.name, error: "Processing" },
        });

        const analysisData = await analyzeBmsScreenshot(
          file,
          forceReanalysis,
          state.primarySystemId,
          useAsync,
        );

        log("info", "Processing analysis result.", {
          fileName: file.name,
          useAsync,
        });
        ...
        const tempRecord: AnalysisRecord = {
          ...
          systemId:
            analysisData.systemId ||
            (state.primarySystemId && state.primarySystemId !== "all"
              ? state.primarySystemId
              : undefined) ||
            undefined,
        ...
        try {
          ...
          log("info", "Updated local cache with new analysis record", {
            id: tempRecord.id,
          });
        } catch (err) {
          log("warn", "Failed to update local cache with new record", {
        ...
        log("info", "Dispatching UPDATE_BULK_JOB_COMPLETED", {
          ...
          analysisKeys: tempRecord.analysis
            ? Object.keys(tempRecord.analysis).length
            : 0,
        });
      ...
      for (const { file, error } of errors) {
        ...
        dispatch({
          type: "UPDATE_BULK_UPLOAD_RESULT",
          payload: { fileName: file, error: `Failed: ${errorMessage}` },
        });
      }
      ...
      const error =
        err instanceof Error
          ? err.message
          : "Failed during bulk analysis submission.";
  ...
  const handleMergeSystems = async () => {
    ...
    log("info", "Starting system merge.", {
      primarySystemId,
      count: selectedSystemIds.length,
    });
    ...
    try {
      ...
      const error =
        err instanceof Error ? err.message : "Failed to merge systems.";
    ...
    setConfirmation({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          log(
            "info",
            "History record deleted successfully (optimistic UI update).",
            { recordId },
          );
  ...
  const handleLinkRecord = async (record: AnalysisRecord) => {
    ...
    try {
      if (systemId === "--create-new--") {
        ...
        throw new Error(
          "Please create the system first, then link the record.",
        );
      } else {
        await linkAnalysisToSystem(
          record.id,
          systemId,
          record.hardwareSystemId,
        );
      ...
      const error =
        err instanceof Error ? err.message : "Failed to link record.";
      log("error", "Failed to link record.", {
        recordId: record.id,
        systemId,
        error,
      });
  ...
  const handleSaveSystem = async (system: BmsSystem) => {
    ...
    try {
      ...
      const error =
        err instanceof Error ? err.message : "Failed to save system.";
      log("error", "Failed to save system.", {
        systemId: systemId || null,
        error,
      });
      dispatch({ type: "SET_ERROR", payload: error });
    ...
    if (options.requiresConfirm) {
      setConfirmation({
        ...
        message:
          ...
          `Are you sure you want to perform the action: ${actionName}?`,
        onConfirm: async () => {
          setConfirmation({ isOpen: false, message: "", onConfirm: () => {} });
          await executeAction();
        },
  ...
  const handleConfirmDeletion = async () => {
    const idsToDelete = duplicateSets.flatMap((set) =>
      set.slice(1).map((record) => record.id),
    );
  ...
  const handleDeleteUnlinked = () =>
    handleGenericAction(
      ...
      "history",
      {
        ...
        confirmMessage:
          "Are you sure you want to delete ALL history records not linked to a system? This is irreversible.",
      },
    );
  ...
  const handleClearHistory = () =>
    handleGenericAction(
      ...
      "history",
      {
        requiresConfirm: true,
        confirmMessage:
          "Are you sure you want to clear ONLY the analysis history store? This is irreversible.",
      },
    );
    ...
    try {
      ...
      if (count > 0) {
        const confirmed = window.confirm(
          `${count} records need weather data. This will process up to 50 records per run. Multiple runs may be needed. Continue?`,
        );
        if (confirmed) {
          ...
          try {
            ...
            const message =
              result.message ||
              `Processed ${result.processedCount || result.updatedCount} records. ${result.updatedCount} updated, ${result.errorCount || 0} errors.`;
            alert(
              message +
            ...
            const error =
              err instanceof Error
                ? err.message
                : "Failed to backfill weather data.";
      ...
      const error =
        err instanceof Error
          ? err.message
          : "Failed to count records needing weather data.";
  ...
  const handleHourlyCloudBackfill = async () => {
    const confirmed = window.confirm(
      "This will fetch hourly weather data for up to 10 days per run. Multiple runs may be needed for full history. This uses weather API calls. Continue?",
    );
    if (confirmed) {
      ...
      try {
        ...
        const message =
          result.message ||
          `Processed ${result.processedDays} days. ${result.hoursInserted} hours inserted, ${result.errors} errors.`;
        alert(
          message +
            (result.completed === false
        ...
        const error =
          err instanceof Error
            ? err.message
            : "Failed to backfill hourly cloud data.";
    ...
    handleGenericAction(
      ...
      async () => {
        ...
        while (result.success && loops < MAX_LOOPS) {
          ...
          log("info", `Auto-associate timed out. looping...`, {
            loop: loops,
            skip,
            totalAssociated,
          });
    ...
    <div className="bg-neutral-dark min-h-screen text-neutral-light p-4 sm:p-6 md:p-8">
      ...
      <main className="space-y-12">
        ...
        <section>
          <h2 className="text-2xl font-semibold text-secondary mb-4 border-b border-gray-600 pb-2">
            Historical Analysis
            {state.isCacheBuilding && (
              <span className="text-sm font-normal text-gray-400 ml-4">
                {" "}
                (Building full chart data: {historyCache.length} records
                loaded...)
              </span>
            )}
          </h2>
          <div className="bg-gray-800 p-4 rounded-lg shadow-inner">
            {loading && historyCache.length === 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-400 min-h-[600px]">
                <SpinnerIcon className="w-8 h-8 text-secondary" />{" "}
                <span className="ml-4">
                  Loading historical data for chart...
                </span>
              </div>
            )}
          </div>
        ...
        {loading && systems.length === 0 && history.length === 0 ? (
          ...
          <>
            <SystemsTable
              ...
              onMergeRequested={async (
                systemIds: string[],
                primaryId: string,
              ) => {
                dispatch({
                  type: "SET_SELECTED_SYSTEM_IDS",
                  payload: systemIds,
                });
            ...
            <section id="solar-integration-section">
              ...
              <SolarIntegrationDashboard
                ...
                systemConfig={(() => {
                  const s =
                    systems.find((sys) => sys.id === primarySystemId) ||
            ...
            <DataManagement
              state={{
                ...state,
                systems: allSystems.length > 0 ? allSystems : state.systems,
              }} // Overlay allSystems for dropdowns
              dispatch={dispatch}
