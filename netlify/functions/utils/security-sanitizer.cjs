...
/**
 * @typedef {Object} SanitizePromptResult
 * @property {string|null} sanitized
 ...
 * @param {Logger} log - Logger instance
 * @returns {SanitizePromptResult} { sanitized: string, warnings: string[] }
 */
...
function sanitizeInsightsRequest(body, log) {
  ...
  if (typeof body.insightMode === 'string') {
    // Allow insightMode for determining analysis strategy
    sanitized.insightMode = sanitizeString(body.insightMode, { maxLength: 50 });
  }
